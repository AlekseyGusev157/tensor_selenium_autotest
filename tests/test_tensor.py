...
def test_download_plugin(web_browser):
    ...
    uploaded_file = os.path.join(os.path.expanduser('~/Downloads'), file_name)
    while True:
    ...
    actual_size_mb = round(os.path.getsize(uploaded_file) / (1024 * 1024), 2)
    assert actual_size_mb == 20.82


def test_send_request(web_browser):
